...
- **Studio Now Actually Reaches the Real Local Model**: The Studio's `/api/ask` route previously spawned `nepali ask` with a **3-second kill timeout** — but loading a real Qwen GGUF model takes 20-60s+, so the genuine neural model almost always timed out and the Studio silently served the hardcoded baked-in engine instead. Now the route only invokes the CLI when a real GGUF model + tokenizer are verified on disk (`NEPALI_AI_MODEL_PATH`/`~/.nepali-ai/...`), and when they are, it waits up to **180s** so the real model genuinely loads and answers. No model configured → the baked-in engine is used directly and honestly (no wasted timeout). The response's `⚡ engine` label now distinguishes `नेपाली नेटिभ एआई (स्थानीय Qwen GGUF मोडेल)` from the baked-in engine.
